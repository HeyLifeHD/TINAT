...
anno <- colData(vst)
dds <-readRDS(file.path(results.dir, "dds.rds"))
anno <- readRDS("/omics/groups/OE0219/internal/tinat/210920_SRP332559_processing_deNovo/gffCompare.annotated.sorted_repeat_anno.rds")
...
averageCounts <-data.frame(DMSO_mean=rowMeans(counts[,rownames(pheno[pheno$treatment=="DMSO",])]), 
    AZA_mean=rowMeans(counts[,rownames(pheno[pheno$treatment=="AZA",])]),
   ITF_mean=rowMeans(counts[,rownames(pheno[pheno$treatment=="ITF",])]),
   ITFandAZA_mean=rowMeans(counts[,rownames(pheno[pheno$treatment=="ITFandAZA",])]))
...
#adjust direction
DEG_results_list$ITF_vs_DMSO <- DEG_results_list$DMSO_vs_ITF
DEG_results_list$ITF_vs_DMSO$log2FoldChange <- -(DEG_results_list$ITF_vs_DMSO$log2FoldChange)
DEG_results_list$DMSO_vs_ITF <- NULL

DEG_results_list$AZA_vs_DMSO <- DEG_results_list$DMSO_vs_AZA
DEG_results_list$AZA_vs_DMSO$log2FoldChange <- -(DEG_results_list$AZA_vs_DMSO$log2FoldChange)
DEG_results_list$DMSO_vs_AZA <- NULL

DEG_results_list$ITFandAZA_vs_DMSO <- DEG_results_list$DMSO_vs_ITFandAZA
DEG_results_list$ITFandAZA_vs_DMSO$log2FoldChange <- -(DEG_results_list$ITFandAZA_vs_DMSO$log2FoldChange)
DEG_results_list$DMSO_vs_ITFandAZA <- NULL
...
#heatmap
mat_genes<- assay(vst_b_replicate)
