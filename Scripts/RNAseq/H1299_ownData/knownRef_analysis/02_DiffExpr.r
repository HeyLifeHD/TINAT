...
anno_original_gene <- anno_original[anno_original$type =="gene",]
anno_original_gene_sub <- unique(as.data.frame(anno_original_gene)[, c("seqnames", "start", "end","strand","gene_id", "gene_type", 
  "gene_name", "dist_nearest_repeat", "nearest_repeat_repName", 
  "nearest_repeat_repClass", "nearest_repeat_repFamily", "nearest_LTR12repeat_repName","dist_nearest_LTR12repeat" )])
DEG_results_list <- lapply(DEG_results_list, function(x){
  x <- dplyr::left_join(x, anno_original_gene_sub, by="gene_id")
...
CTA <-  as.data.frame(data.table::fread(file.path("/omics/groups/OE0219/internal/tinat/external_data/",  "Cancer_Testis_Antigens.txt"), header=FALSE))
...
DEG_results_list_plot <- lapply(DEG_results_list_plot, function(x){
  ...
  temp <-head(x[x$gene_name %in% CTA[,1],]$gene_name,5)
  x$SYMBOL <- NA
  x[x$gene_name %in% temp,]$SYMBOL <- x[x$gene_name %in% temp,]$gene_name
...
Volcanos<- list() 
for (i in names(DEG_results_list_plot)){
  Volcanos[[i]] <- ggscatter(DEG_results_list_plot[[i]], x ="log2FoldChange", y = "padj",title=i,
          xlab="Gene expression (log2 fold change)",#xscale="log10" ,
          ylab="- log10(P.adjusted)", #xlim=c(-0.5,0.5),
          color = "col" ,shape = 16, size = 1.5, palette=col, # Points color, shape and size
          add.params = list(color = "grey", fill = "lightgray"), legend.title="Significance",# main=i# Customize reg. line
...
nrow(DEG_results_list$DACandSB939_vs_DMSO[which(DEG_results_list$DACandSB939_vs_DMSO$padj < 0.05 & DEG_results_list$DACandSB939_vs_DMSO$log2FoldChange>5),])
paste0(DEG_results_list$DACandSB939_vs_DMSO[which(DEG_results_list$DACandSB939_vs_DMSO$padj < 0.05 & DEG_results_list$DACandSB939_vs_DMSO$log2FoldChange>5),]$gene_name, collapse=", ")
