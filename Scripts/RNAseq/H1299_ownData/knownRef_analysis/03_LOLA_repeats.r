...
anno_gene <- anno_original[anno_original$type == "gene",]
userUnisverse <- resize(anno_gene,1)
...
for(i in names(DEG_results_list)){
    ...
    UserSet <- list(Upregulated=resize(anno_gene[anno_gene$gene_id %in%rownames(DEG_results_list[[i]][which(DEG_results_list[[i]]$padj < alpha & DEG_results_list[[i]]$log2FoldChange>lfc),]),],1),
        Downregulataed=resize(anno_gene[anno_gene$gene_id %in%rownames(DEG_results_list[[i]][which(DEG_results_list[[i]]$padj < alpha & DEG_results_list[[i]]$log2FoldChange<(-lfc)),]),],1))
