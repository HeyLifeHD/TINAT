...
ORF_stats <- ORF_stats[order(ORF_stats$width, decreasing=TRUE),]
ORF_stats$order <- 1:nrow(ORF_stats)
#plot as rainfall plot
plot <- ggplot(ORF_stats, aes(x=width, y=order))+
    theme_pubr()+
    geom_point(data = subset(ORF_stats, candidate== "no_candidate" ), 
        fill = "gray",color = "gray", alpha = 0.5, size=.1) +
    geom_point(data = subset(ORF_stats, candidate== "candidate" ), 
        fill = comp_col[ "DACandSB939_vs_DMSO"],color = comp_col[ "DACandSB939_vs_DMSO"], alpha = 1, size=.5)+
    geom_vline(xintercept=mean(ORF_stats[ORF_stats$candidate=="candidate",]$width)  , linetype = 2, color=comp_col[ "DACandSB939_vs_DMSO"]) +
    annotate(geom="text", x=mean(ORF_stats[ORF_stats$candidate=="candidate",]$width)+10, y=median(ORF_stats$order), 
        label=paste0("Mean ORF length: ", round(mean(ORF_stats[ORF_stats$candidate=="candidate",]$width)), " AAs"), color=comp_col[ "DACandSB939_vs_DMSO"]) +
    geom_vline(xintercept=mean(ORF_stats[ORF_stats$candidate=="no_candidate",]$width)  , linetype = 2, color="gray") +
    annotate(geom="text", x=mean(ORF_stats[ORF_stats$candidate=="no_candidate",]$width)+10, y=median(ORF_stats$order )+10000, 
        label=paste0("Mean ORF length: ", round(mean(ORF_stats[ORF_stats$candidate=="no_candidate",]$width)), " AAs"), color="gray") +
    scale_x_log10()+
...
rasterize(plot, layers="Point", dpi=100)
dev.off()
...
ggplot(ORF_stats, aes(x=width, y=order))+
    theme_pubr()+
    geom_point(data = subset(ORF_stats, candidate== "no_candidate" ), 
        fill = "gray",color = "gray", alpha = 0.5, size=.1) +
    geom_point(data = subset(ORF_stats, candidate== "candidate" ), 
        fill = comp_col[ "DACandSB939_vs_DMSO"],color = comp_col[ "DACandSB939_vs_DMSO"], alpha = 1, size=.5)+
    geom_vline(xintercept=median(ORF_stats[ORF_stats$candidate=="candidate",]$width)  , linetype = 2, color=comp_col[ "DACandSB939_vs_DMSO"]) +
    annotate(geom="text", x=median(ORF_stats[ORF_stats$candidate=="candidate",]$width)+10, y=median(ORF_stats$order), 
        label=paste0("Median ORF length: ", round(median(ORF_stats[ORF_stats$candidate=="candidate",]$width)), " AAs"), color=comp_col[ "DACandSB939_vs_DMSO"]) +
    geom_vline(xintercept=median(ORF_stats[ORF_stats$candidate=="no_candidate",]$width)  , linetype = 2, color="gray") +
    annotate(geom="text", x=median(ORF_stats[ORF_stats$candidate=="no_candidate",]$width)+10, y=median(ORF_stats$order )+10000, 
        label=paste0("Median ORF length: ", round(median(ORF_stats[ORF_stats$candidate=="no_candidate",]$width)), " AAs"), color="gray") +
    scale_x_log10()+   
    xlab("Length of ORF [AA]") + ylab("Order by length")
dev.off()
