...
for( i in names(genes2plot)){
    ...
    class_plot <- ggbarplot(trans_class, x="Var1", y="Freq",
        ...
        rremove("legend") +
        rremove("ylab")
    pdf(file.path(output.dir,i,  "transcript_classification_simplified.pdf"), height=5, width=5 )
    print(class_plot)
    dev.off()
    ...
    pdf(file.path(output.dir,i, "transcript_classification_simplified_pie.pdf"), height=5, width=5)
    ...
    exon_plot <- ggboxplot(anno_classi_sub, x="class_code_simple", y="num_exons",
        ...
        coord_flip() +
        ylim(c(0,40))
    pdf(file.path(output.dir,i, "transcript_exon_number_simplified.pdf"), height=5, width=5)
    print(exon_plot)
    dev.off()
    ...
    pdf(file.path(output.dir,i, "Exon_class.pdf"), width=5)
        print(pies)
    dev.off()
    ...
    length_plot <- ggboxplot(anno_classi_sub, x="class_code_simple", y="width",
        ...
        coord_flip() +#
    ylim(c(0,200))
    pdf(file.path(output.dir,i, "transcript_length_simplified.pdf"), height=5, width=5)
    print(length_plot)
    dev.off()

    #combined plot
    pdf(file.path(output.dir,i, "transcript_anno_classCode_simple_length_exonNum_combined.pdf"), width=10, height=5)
    ...
    dev.off()
    pdf(file.path(output.dir,i, "transcript_anno_classCodePie_simple_length_exonNum_combined.pdf"), width=10, height=5)
    ...
    pie_eres <- ggarrange(pie[[trans_class[order(trans_class$Freq, decreasing=TRUE),]$Var1[3]]],
        pie[[trans_class[order(trans_class$Freq, decreasing=TRUE),]$Var1[2]]], pie[[trans_class[order(trans_class$Freq, decreasing=TRUE),]$Var1[1]]],
        labels=NULL,
        common.legend = TRUE,
        ncol = 1, nrow = 3
        )
    pdf(file.path(output.dir,i, "Pie_transcript_class_ERE_anno.pdf"), width=10, height=10)
    print(pie_eres)
    dev.off()


    pdf(file.path(output.dir, i,"transcript_anno_classCodePie_simple_length_exonNum_exonPies_ere_anno_combined_v2.pdf"), width=15, height=5)
    ...
    tpm_allTranscripts <- ggboxplot(tpm_mean_h1299_merged_melt, x="variable", y="value",
        ...
        coord_flip() +
        ylim(c(0,10))
    pdf(file.path(output.dir, i,"transcript_classification_TPM.pdf"), height=5, width=5 )
