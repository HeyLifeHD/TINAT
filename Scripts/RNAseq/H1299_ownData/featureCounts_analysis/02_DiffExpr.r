...
DEG_results_list_plot <- lapply(DEG_results_list_plot, function(x){
  ...
  temp <- c(head(row.names(x[x$col=="red",]),5), head(row.names(x[x$col=="blue",]), 5))
  x$SYMBOL <- NA
...
#meta
nrow(DEG_results_list$LTR12C_wG_vs_NonTargeting[which(DEG_results_list$LTR12C_wG_vs_NonTargeting$padj < 0.05 & DEG_results_list$LTR12C_wG_vs_NonTargeting$log2FoldChange>1),])
paste0(DEG_results_list$LTR12C_wG_vs_NonTargeting[which(DEG_results_list$LTR12C_wG_vs_NonTargeting$padj < 0.05 & DEG_results_list$LTR12C_wG_vs_NonTargeting$log2FoldChange>1),]$gene_name, collapse=", ")
paste0(DEG_results_list$LTR12C_wG_vs_NonTargeting[which(DEG_results_list$LTR12C_wG_vs_NonTargeting$padj < 0.05 & DEG_results_list$LTR12C_wG_vs_NonTargeting$log2FoldChange<(-1)),]$gene_name, collapse=", ")
