...
output.dir <- "/omics/groups/OE0219/internal/tinat/220316_cellline_deNovo_assembly/visualizations_custom"
...
anno_classi_h1299 <- as.data.frame(data.table::fread("/omics/groups/OE0219/internal/tinat/210726_shortRead_processing_deNovo_custom4/gffCompare.mergedTranscripts.gtf.tmap"))
...
#same but with h1299 assembly numbers
#get numbers of novel genes and transcripts
numb_trans_deNovoH12 <- length(unique(anno_classi_h1299$qry_id))
numb_genes_deNovoH12 <- length(unique(anno_classi_h1299$qry_gene_id))
gene_trans_numbers <- data.frame(assembly=c("genecode","deNovo H1299","deNovo cell line panel"),
    genes=c(numb_genes_genecode, numb_genes_deNovoH12, numb_genes_deNovo),
    transcripts=c(numb_trans_genecode, numb_trans_deNovoH12, numb_trans_deNovo))
gene_trans_numbers <- reshape2::melt(gene_trans_numbers)
gene_trans_numbers$assembly <- factor(gene_trans_numbers$assembly, ordered = TRUE, 
levels = (c("genecode","deNovo H1299","deNovo cell line panel")))
glob_col <-  c("gray", col[c(9,10)])
names(glob_col)<- c("genecode","deNovo H1299","deNovo cell line panel")

pdf(file.path(output.dir, "number_genes_transcripts_inclH1299_log10.pdf"), height=5, width=5)
ggbarplot(gene_trans_numbers, x="variable", y="value",
    fill="assembly", palette=glob_col,yscale="log10",#color="assembly", 
    ylab="Number", label=TRUE, position = position_dodge(0.8),
    legend.title="Assembly",
    ...
    rremove("xlab")
dev.off()
pdf(file.path(output.dir, "number_genes_transcripts_inclH1299.pdf"), height=5, width=5)
ggbarplot(gene_trans_numbers, x="variable", y="value",
    fill="assembly", palette=glob_col,# color="assembly",
    ylab="Number", label=TRUE, position = position_dodge(0.8),
    legend.title="Assembly",
...
pie_ere <- list()
    for(j in names(anno_classi_ere_split_stats)){
        ...
        pie_ere[[j]] <- ggpie(anno_classi_ere_split_stats[[j]], "Freq", #label = labs,
                lab.pos = "in", lab.font = "black", palette=ere_col,#label.size = 16,
                fill = "repeat_class", color = "black")+rremove("legend.title")
    }

pie_eres <- ggarrange(pie_ere$"non-chimeric (novel)",
        pie_ere$"chimeric (novel)",
        pie_ere$"known",labels=NULL,
        common.legend = TRUE,
        ncol = 1, nrow = 3
        )
