...
contrasts <- as.data.frame(combn(as.character(unique(colData(dds)$grouping)), 2))
contrasts <- contrasts[c(2,1),]
results <- list()
for (i in 1:length(contrasts)) {
  results[[i]]<- results(dds, contrast=c("grouping",as.character(contrasts[1,i]),  as.character(contrasts[2,i])), alpha = alpha, lfcThreshold = lfc) #extract results of wanted comparison
...
pheno <- colData(dds)
# averageCounts <-data.frame(DMSO_mean=rowMeans(counts[,rownames(pheno[pheno$grouping=="DMSO",])]), 
#     DAC_mean=rowMeans(counts[,rownames(pheno[pheno$grouping=="DAC",])]),
#     SB939_Rep3_mean=rowMeans(counts[,rownames(pheno[pheno$grouping=="SB939",])]),
#     DACandSB939_Rep3_mean=rowMeans(counts[,rownames(pheno[pheno$grouping=="DACandSB939",])]))
...
for (i in names(genes2plot)){
  ...
  annorow <- rownames(plot)
    ...
    heat[[i]]<- pheatmap(plot[,     rownames(anno[anno$grouping %in% comp_oi,])], 
...
#     anno_colors <- list(grouping=col, cluster=color_cluster)
#     #heatmap
#     p <- pheatmap(plot, scale="row", show_colnames=F,color= c(hcl.colors(20,"Blues"),rev(hcl.colors(20,"Reds"))),#labels_row=annorow,
#                         annotation_col=as.data.frame(annovst),
#                         annotation_colors=anno_colors, show_rownames=F, clustering_distance_rows="correlation" ,
...
#   clust_expr_mean[[i]]<- data.frame(AverageExpression=  clust_expr_mean[[i]], grouping=as.character(anno$grouping))
...
# ggboxplot(clust_expr_mean_unlist, x="grouping", y="AverageExpression", 
#   color="grouping", facet.by="cluster", palette=col) +
#   rotate_x_text(angle = 90) + stat_compare_means(label.y = .1, size=2.5)
# dev.off()
# #in 2 row visualization
# g <- ggboxplot(clust_expr_mean_unlist, x="grouping", y="AverageExpression", order=c("DMSO", "DAC", "SB939","DACandSB939"),
#   color="grouping", palette=col) +
#   rotate_x_text(angle = 90) + stat_compare_means(label.y = .1, size=2.5)
