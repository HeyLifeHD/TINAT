...
DEG_results_list_plot <- lapply(DEG_results_list, function(x){
  ...
  x$col <- ifelse(test = x$padj < alpha & abs(x$log2FoldChange)>lfc, yes = "significant", 
                  no ="not_significant")
  x$col <- ifelse(test = x$gene_name %in% anno_classi_known_HLA$ref_gene_id, yes = "HLA", 
                  no =x$col)
  x$Label <- NA
  x[x$gene_name %in% anno_classi_known_HLA$ref_gene_id,][1:5,]$Label <- x[x$gene_name %in% anno_classi_known_HLA$ref_gene_id,][1:5,]$gene_name
  x$pvalue <-(-(log10(x$pvalue)))
  x$padj <- (-(log10(x$padj)))
  x
}) 
...
for (i in c( "DAC_vs_DMSO","SB939_vs_DMSO","DACSB_vs_DMSO")){
  Volcanos[[i]] <- ggplot(DEG_results_list_plot[[i]], aes(x=log2FoldChange, y=padj))+
    ...
    geom_point(data = subset(DEG_results_list_plot[[i]], class_code_simple== "non-chimeric (novel)" & col=="significant"), 
        fill = class_col[ "non-chimeric (novel)"],color = class_col[ "non-chimeric (novel)"], alpha = 1, size=.8) +
    geom_point(data = subset(DEG_results_list_plot[[i]],  col=="HLA"), 
        fill = "red",color = "red", alpha = 1, size=.8) +
    ggrepel::geom_text_repel(aes(label = Label),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
                  segment.color = 'grey50') +
  ...
  ggsave(plot=ggrastr::rasterize(Volcanos[[i]], layers="Point", dpi=500),file.path(PostDE.dir,i,"volcano_Class_code_simple_HLALabel.pdf"),height = 5, width = 5, useDingbats = FALSE)
  ggsave(plot=ggrastr::rasterize(Volcanos[[i]], layers="Point", dpi=500),file.path(PostDE.dir,i,"volcano_Class_code_simple_HLALabel.png"),height = 5, width = 5, device="png")
  print(i)
}
