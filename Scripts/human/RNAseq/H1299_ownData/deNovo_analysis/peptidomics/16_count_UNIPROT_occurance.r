...
pattern_match_1MM <- lapply(peptides_oi, function(x){
    pattern_match <- vmatchPattern(x, uniprot,max.mismatch=1)
    unlist(pattern_match )
})
#2 mismatch
pattern_match_2MM <- lapply(peptides_oi, function(x){
    pattern_match <- vmatchPattern(x, uniprot,max.mismatch=2)
    unlist(pattern_match )
})
#3 mismatch
pattern_match_3MM <- lapply(peptides_oi, function(x){
    pattern_match <- vmatchPattern(x, uniprot,max.mismatch=3)
    unlist(pattern_match )
})
...
pattern_match_df <- read.table( file.path(output.dir, "2of3DACSB_unique", "peptide_blast_uniprot.txt"),header=TRUE, sep="\t")
anno_original <-  readRDS("/omics/groups/OE0219/internal/tinat/raw_data_repo/references/gencode.v29lift37.annotation.repeat_anno.rds")
temp <- sapply(strsplit(as.character(pattern_match_df$names), "|", fixed=TRUE), "[", 2)
pattern_match_df$Protein <- temp
#temp <- sapply(strsplit(as.character(pattern_match_df$names), "|", fixed=TRUE), "[", 3)
#temp <- sapply(strsplit(temp, "_HUMAN", fixed=TRUE), "[", 1)
evidence_anno <- artMS::artmsAnnotationUniprot(x = data.frame(Protein=temp),
                                         columnid = 'Protein',
                                         species = 'human')
temp <- left_join(data.frame(Protein=temp), evidence_anno)                 
#pattern_match_df$gene_name <- evidence_anno$Gene
pattern_match_df <- left_join(pattern_match_df, temp)
pattern_match_df$gene_name <- pattern_match_df$Gene
pattern_match_df$gene_name[pattern_match_df$gene_name  %in% anno_original$gene_name]
pattern_match_df$gene_name[!pattern_match_df$gene_name  %in% anno_original$gene_name]

pattern_match_df_anno <- dplyr::left_join(pattern_match_df[,!colnames(pattern_match_df)%in% c("start", "end", "width")], as.data.frame(anno_original)[which( anno_original$type == "gene"),])
write.table(pattern_match_df_anno, file.path(output.dir, "2of3DACSB_unique", "number_of_mismatches_uniprot_annotated.txt"),row.names = FALSE, col.names=TRUE, quote=FALSE, sep="\t")
...
pie_eres <- ggarrange(pie[[1]], pie[[2]], 
    labels=paste0(names(pie), " Mismatches"),
    common.legend = TRUE,
    ncol = 1, nrow = 3
    )
pdf( file.path(output.dir, "2of3DACSB_unique", "number_of_mismatches_uniprot_annotated_Pie_transcript_class_ERE_anno.pdf"), width=5, height=10)
print(pie_eres)
dev.off()
...
pdf( file.path(output.dir, "2of3DACSB_unique", "number_of_mismatches_uniprot_annotated_Pie_transcript_class_LTR_anno.pdf"), width=5, height=10)
print(pie_eres)
dev.off()
