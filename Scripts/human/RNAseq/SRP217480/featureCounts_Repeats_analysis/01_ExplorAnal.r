...
#anno
col<-grDevices::colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)]
col <- sample(col, length(unique(anno$grouping)))
...
names(col) <- levels(colData(dds)$grouping)
pdf(file.path(PreDE.dir, "PCA12.pdf"), height = 5, width = 14)
...
percentvarvst <- round(100* attr(pcaDatavst, "percentVar"))
pdf(file.path(PreDE.dir, "PCA12_5000mvGenes.pdf"), height = 5, width = 14)
ggscatter(pcaDatavst, x="PC1", y="PC2",
            size=5,
            color = "grouping", 
           # label= "replicate",
...
pdf(file.path(PreDE.dir, "PCA23.pdf"), height = 5, width = 14)
...
percentvarvst.alt <- round(100* attr(pcaDatavst.alt, "percentVar"))
pdf(file.path(PreDE.dir, "PCA23_5000mvGenes.pdf"), height = 5, width = 14)
ggscatter(pcaDatavst.alt, x="PC2", y="PC3",            
            size=5,
            color = "grouping", 
            repel=TRUE,
...
dev.off()
pdf(file.path(PreDE.dir,"Heatmap_noScale_LTR12repeats_2.pdf"),height= 7)
pheatmap(vst_LTR, scale="row", show_colnames=F,color= c(hcl.colors(20,"Blues"),rev(hcl.colors(20,"Reds"))),labels_row=rownames(vst_LTR),
annotation_col=as.data.frame(anno)[,1:5], show_rownames=T)
dev.off()
